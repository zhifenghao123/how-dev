 ...
 * 临时密钥签名测试客户端
 *
 * <p>模拟客户端使用临时密钥按照签名方法的规范构造签名并发送请求，
 * 用于验证服务端对临时密钥签名验证逻辑的正确性。</p>
 *
 * <h3>使用前提</h3>
 * <ol>
 *   <li>服务端已启动（默认 http://localhost:8080）</li>
 *   <li>已通过 /user/tempAccessKey/apply 接口申请了临时密钥</li>
 *   <li>将申请到的 AccessKeyId、SecretAccessKey、SessionToken 填入下方常量</li>
 * </ol>
 *
 * <h3>与持久密钥签名的区别</h3>
 * <p>临时密钥签名在请求头中额外携带 X-Session-Token，服务端据此判断密钥类型并进行相应的验签处理。</p>
 *
 * <h3>签名流程</h3>
 * <pre>
 ...
 * 6. 在请求头中附加 X-Session-Token
 * 7. 发送 HTTP 请求
 * </pre>
 */
public class TempKeySignatureClient {
    ...
    /**
     * 临时密钥 AccessKeyId - 通过 /user/tempAccessKey/apply 接口申请获得
     * <p>请替换为实际的 AccessKeyId</p>
     */
    private static final String ACCESS_KEY_ID = "";

    /**
     * 临时密钥 SecretAccessKey - 申请时返回的明文密钥
     * <p>请替换为实际的 SecretAccessKey</p>
     */
    private static final String SECRET_ACCESS_KEY = "";

    /**
     * 临时密钥 SessionToken - 申请时返回的会话令牌
     * <p>请替换为实际的 SessionToken</p>
     */
     ...
     * <ol>
     *   <li>准备请求参数</li>
     *   <li>按规范构建签名（与持久密钥签名流程一致）</li>
     *   <li>组装 Authorization 头</li>
     *   <li>附加 X-Session-Token 头</li>
    ...
    public static void sendSignedPostRequest(String uri, String requestBody) throws Exception {
        ...
        System.out.println("========== 临时密钥签名客户端签名流程 ==========");
        System.out.println("请求时间戳: " + timestamp);
        System.out.println("请求方法: " + httpMethod);
        System.out.println("请求URI: " + uri);
        System.out.println("请求体: " + requestBody);
        System.out.println("SessionToken: " + SESSION_TOKEN.substring(0, Math.min(32, SESSION_TOKEN.length())) + "...");
        System.out.println();
        ...
        // ========== 第五步：发送 HTTP 请求（附加 X-Session-Token） ==========
        ...
        // 临时密钥特有：附加 SessionToken
        connection.setRequestProperty("X-Session-Token", SESSION_TOKEN);
    ...
    public static void sendSignedGetRequest(String uri, TreeMap<String, String> queryParams) throws Exception {
        ...
        System.out.println("========== 临时密钥 GET 请求签名流程 ==========");
        System.out.println("请求时间戳: " + timestamp);
        System.out.println("请求URI: " + uri);
        System.out.println();
     ...
     * <ol>
     *   <li>服务端已启动</li>
     *   <li>已通过 /user/tempAccessKey/apply 接口申请临时密钥</li>
     *   <li>已替换 ACCESS_KEY_ID、SECRET_ACCESS_KEY、SESSION_TOKEN 为实际值</li>
     * </ol>
     */
    public static void main(String[] args) {
        try {
            // ==================== 测试1：临时密钥 POST 请求签名验证 ====================
            System.out.println("==================== 测试1：临时密钥 POST 请求签名验证 ====================\n");
            String postBody = "{\"data\":\"hello, temp key signature!\"}";
            sendSignedPostRequest("/sign/verify", postBody);

            System.out.println("\n");

            // ==================== 测试2：临时密钥 GET 请求签名验证 ====================
            System.out.println("==================== 测试2：临时密钥 GET 请求签名验证 ====================\n");
